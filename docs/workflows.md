...
- **每日链路**：`scripts/cron_ai_video_enhancer_daily.sh` 会在 `run_video_enhancer.py` 成功结束后追加执行 TopN 推送，默认读取项目 `.env` 的 `FEISHU_TEST_WEBHOOK` 推送测试群；未配置时跳过追加卡片，不影响多维表同步和原 VE 日报。
- **数据源**：默认读取 `VIDEO_ENHANCER_BITABLE_URL` 指向的 VE 主表；目标日素材来自「抓取日期」，历史反馈默认从 `2026-05-25` 到目标日前一天，且只使用浩鹏 `采纳 / 入素材库 / 不采纳 / 重复抓取 / 删除 / 拒绝` 等有效反馈，`待定` 不作为正负样本。
