...
支持两种 pull_spec 模式：
  - latest_yesterday：UI 指定日期 target_date~target_date + 最新创意 + 仅保留 first_seen=目标日
...
from ua_workflows.shared.config import CONFIG_DIR, DATA_DIR, PROJECT_ROOT
...
from ua_workflows.shared.guangdada.competitor_utils import advertiser_matches_product
from ua_workflows.shared.db.arrow2 import dedupe_arrow2_raw_items_by_ad_key
...
def _empty_product_result(entry: dict, target_date: str, pull_id: str, error: str = "") -> dict:
...
async def main() -> None:
    ...
    parser.add_argument(
        "--pause-after-setup",
        action="store_true",
        help="登录并设置筛选后暂停，方便人工检查页面控件；不会继续爬取或写 raw",
    )
    parser.add_argument(
        "--output-prefix",
        default="",
    ...
    parser.add_argument(
        "--ui-date-range",
        action="store_true",
        help="兼容参数：latest_yesterday 默认已同步设置 UI 日期为 target_date ~ target_date。",
    )
    parser.add_argument(
        "--no-ui-date-range",
        action="store_true",
        help="调试回退：latest_yesterday 仍按旧口径先选 7天，再本地按 first_seen 筛目标日。",
    )
    ...
    ui_date_range = (
        (target_date, target_date)
        if filter_yesterday and not bool(args.no_ui_date_range)
        else None
    )
    ...
    if ui_date_range:
        print(f"UI 日期范围: {ui_date_range[0]} ~ {ui_date_range[1]}")
    print(f"输出前缀: {prefix}")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # ─── 一次性设置筛选 ───
            date_desc = "指定日期" if ui_date_range else f"{day_span}天"
            setup_desc = f"{date_desc} / 素材 / {'最新创意' if order_by == 'latest' else '展示估值'}"
            ...
            if not await _do_setup(
                ...
                print(
                    "[失败] 筛选条件未全部成功，中止爬取（不写 raw）",
                    file=sys.stderr,
                )
                sys.exit(3)
            print("[全局] 筛选设置完成\n")
            if args.pause_after_setup:
                print("[暂停] 筛选已设置完成，请在有头浏览器中检查页面；按 Enter 关闭浏览器并退出…")
                try:
                    input()
                except EOFError:
            ...
            for i, entry in enumerate(entries):
                try:
                    ...
                    print(
                        f"[WARN] 产品 {entry['match']!r} 首次爬取失败，将重登并重试一次：{type(e).__name__}: {e}",
                        file=sys.stderr,
                    )
                    try:
                        await _close_modal(page)
                        if not await login(page, _email, _password):
                            raise RuntimeError("重登失败")
                        await _await_post_login_shell(page)
                        await _try_click_search_tab(page, search_tab)
                        if not await _do_setup(
                            page,
                            ...
                            raise RuntimeError("重设筛选失败")
                        result = await _run_one_product(
                            page, entry, target_date, pull_spec, channels, countries, is_tool
                        )
                    except Exception as e2:
                        print(
                            f"[ERROR] 产品 {entry['match']!r} 重试仍失败，跳过该产品继续后续产品：{type(e2).__name__}: {e2}",
                            file=sys.stderr,
                        )
                        result = _empty_product_result(
                            entry, target_date, pull_id, f"{type(e2).__name__}: {e2}"
                        )
            ...
            raw_payload["ui_date_range"] = list(ui_date_range) if ui_date_range else None
