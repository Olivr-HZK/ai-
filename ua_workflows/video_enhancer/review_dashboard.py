...
def _compute_clip_crossday_preview(
    ...
    for appid, rows in by_app.items():
        ...
        _kept, removed = _cluster_clip_dedupe(
            threshold=threshold,
...
def _render_play_asset_library_section(assets: list[dict[str, Any]]) -> str:
    parts = ['<h2 class="section-title">玩法资产库（历史沉淀）</h2>']
    if not assets:
        parts.append('<div class="note">还没有玩法资产定义。</div>')
        return "".join(parts)
    cards: list[str] = []
    for asset in assets:
        tags = [
...
def write_filter_review_dashboard(
    ...
    *,
    ...
    asset_variant_filtered_rows = [
        {
            **item,
            "_review_status": (
                "同资产变种非代表"
                if str(item.get("play_asset_novelty_label") or "") in ("新玩法", "新变种")
                else "已沉淀玩法/变种"
            ),
        }
    ...
    for rows in (kept_rows, new_push_rows, asset_variant_filtered_rows, cluster_only_rows, one_liner_rows):
    ...
    stats = [
        ...
        ("日报新玩法", daily_summary.get("new_play_asset_count", 0), f"新变种 {daily_summary.get('new_play_variant_count', 0)} / 推送代表 {len(new_push_rows)}"),
        ("日报未推送", len(asset_variant_filtered_rows), "同变种非代表或已沉淀玩法"),
    ]
    ...
    def push_extra(item: dict[str, Any], row: dict[str, Any]) -> list[tuple[str, Any]]:
        return [
            ("推送段", item.get("play_asset_novelty_label") or "新玩法/新变种"),
            ("玩法资产", item.get("_play_asset_name") or "待沉淀"),
            ("子标签", item.get("_play_asset_subtags") or "-"),
            ("同变种素材", item.get("cluster_material_count") or item.get("daily_play_cluster_size") or 1),
            ("玩法族", _play_family(row, item)),
        ]
    ...
    daily_funnel_note = (
        ...
        f"→ 新玩法 {daily_summary.get('new_play_asset_count', 0)} 个 "
        f"→ 新玩法变种 {daily_summary.get('new_play_variant_count', 0)} 个 "
        f"→ 推送代表 {len(new_push_rows)} 条；"
        f"未推送 {len(asset_variant_filtered_rows)} 条会在下方展示，其中包含同资产变种非代表和已沉淀玩法/变种。"
    )
    html = f"""<!doctype html>
<html lang="zh-CN">
<head>
...
<body>
  ...
  <main class="wrap">
    <div class="note">这个页面会随 VE pipeline 常态化刷新。 一句话筛选板块已前置，并且按「被剔除素材 vs 对比对象」成对展示封面；“其中封面已覆盖” 不再计作一句话筛选能力，真正需要看玩法量化稳定性的，是“玩法兜底素材”。</div>
    ...
    {_render_material_section(title="日报新玩法 / 新变种推送段", rows=new_push_rows, target_date=target_date, meta=meta, today_meta=today_meta, ribbon="进入推送", ribbon_cls="keep", empty_text="没有日报新玩法或新变种", extra_builder=push_extra)}
    ...
    {_render_cover_section(title="CLIP 封面命中（跨日 / 日内）", groups=clip_groups, mode="CLIP", target_date=target_date, meta=meta, today_meta=today_meta)}
    {_render_cover_section(title="ahash / URL 指纹跨日命中", groups=fingerprint_groups, mode="指纹", target_date=target_date, meta=meta, today_meta=today_meta)}
  </main>
  <script>
