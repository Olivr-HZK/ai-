...
async def _click_day_span(page, day_span: str, log_prefix: str) -> bool:
    """点击时间窗：7 / 30 / 90 天。显式请求 30/90 却点不中时返回 False，不再默默改回 7 天。"""
    ...
    if s in ("30", "30天"):
        for keys in (["label:has-text('30天')", "30天", "时间"], ["input[value='30']"], ["三十天", "时间"]):
            if await _click(page, keys):
                ...
                return True
        print(f"{log_prefix}30天 ✗ 未找到可选中的时间窗", file=sys.stderr)
        return False
    if s in ("90", "90天"):
        for keys in (["label:has-text('90天')", "90天", "时间"], ["input[value='90']"], ["九十天", "时间"]):
            if await _click(page, keys):
                ...
                return True
        print(f"{log_prefix}90天 ✗ 未找到可选中的时间窗", file=sys.stderr)
        return False
    print(f"{log_prefix}[提醒] 未知 day_span={s!r}，尝试点 7 天", file=sys.stderr)
    ok = await _click(page, ["七天", "时间"])
...
async def _select_top_popularity_option(
    ...
    *,
    ...
    """
    展开「Top 创意 / 人气」类下拉：若为 Top10% 等，先点选项 title=「人气值Top10%」；
    否则仍按 popularity_option_text 子串匹配 inner_text，或 use_first_fallback 点首项。
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if await selector.count() > 0:
                ...
                title_attr = _ant_popularity_option_title(want)
                if title_attr:
                    try:
                        opt_by_title = page.locator(
                            f'div.ant-select-item.ant-select-item-option[title="{title_attr}"]'
                        )
                        await opt_by_title.first.wait_for(state="visible", timeout=5000)
                    except Exception:
                        opt_by_title = page.locator(
                            f'div.ant-select-item.ant-select-item-option[title="{title_attr}"]'
                        )
                    if await opt_by_title.count() > 0:
                        tloc = opt_by_title.first
                        await tloc.scroll_into_view_if_needed()
                        try:
                            await tloc.click(timeout=3000)
                        except Exception:
                            await tloc.click(timeout=3000, force=True)
                        _p(
                            f"{log_prefix}  已点 Top 创意 → title={title_attr!r}",
                            log_quiet=log_quiet,
                        )
                        ok = True
                if ok:
...
def _arrow2_apply_post_filters(
    ...
    if prod and str(prod).strip():
        from ua_workflows.shared.guangdada.competitor_utils import advertiser_matches_product  # noqa: PLC0415
    ...
    *,
    ...
    if is_tool:
        ...
        if not tool_ok:
            ...
            print(
                f"{log_prefix}[失败] 需要「工具」页但未能切换，中止。",
                file=sys.stderr,
            )
            return False
        await page.wait_for_timeout(1000)
    ...
    ch_needed = 0
    if ad_channel_labels and not _env_truthy(
        "ARROW2_SKIP_AD_CHANNEL_FILTER", default=False
    ):
        ch_needed = len([x for x in ad_channel_labels if str(x or "").strip()])
    if ch_needed:
        n_ch = await _select_ad_channel_checkboxes(
            page, ad_channel_labels, log_prefix, log_quiet=log_quiet
        )
        if n_ch < ch_needed:
            print(
                f"{log_prefix}[失败] 广告渠道仅勾选 {n_ch}/{ch_needed}，中止。",
                file=sys.stderr,
            )
            return False

    co_needed = 0
    if country_codes and not _env_truthy(
        "ARROW2_SKIP_COUNTRY_FILTER", default=False
    ):
        co_needed = len([x for x in country_codes if str(x or "").strip()])
    if co_needed:
        n_co = await _select_countries_on_page(
            page, country_codes, log_prefix, log_quiet=log_quiet
        )
        if n_co < co_needed:
            print(
                f"{log_prefix}[失败] 国家/地区仅选中 {n_co}/{co_needed}，中止。",
                file=sys.stderr,
            )
            return False
    ...
    if not ok:
        ...
        print(f"{log_prefix}[失败] 未能点到「素材」，中止。", file=sys.stderr)
        return False
    ...
    if popularity_option_text or use_popularity_top1:
        ...
        if not p_ok:
            print(
                f"{log_prefix}[失败] Top 创意下拉未成功，中止。",
                file=sys.stderr,
            )
            return False
        await page.wait_for_timeout(2500)
