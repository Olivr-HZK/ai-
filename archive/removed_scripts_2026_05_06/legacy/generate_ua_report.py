"""
根据 batch_ua_results.json 调用大模型生成 UA 素材日报。

日报内容：
1. 各公司各游戏的 UA 素材介绍
2. 我方可用的 UA 素材灵感分析

用法: python generate_ua_report.py
...
OUTPUT_FILE = REPORTS_DIR / "ua_report_daily.md"


def find_downloaded_files(company: str, game: str, ad_key: str) -> dict:
    """查找已下载的素材文件"""
    import re
    def safe(s: str) -> str:
    ...
    safe_game = safe(game)
    ad_key_short = ad_key[:12]
    files = {"cover": None, "logo": None, "video": None}
    if DOWNLOADS_DIR.exists():
        # 匹配模式：公司_游戏_adkey_类型.扩展名
        patterns = [
            f"{safe_company}_{safe_game}_{ad_key_short}_cover.*",
            f"{safe_company}_{safe_game}_{ad_key_short}_logo.*",
            f"{safe_company}_{safe_game}_{ad_key_short}_video.*",
        ]
...
def build_summary(data: dict) -> tuple[str, dict]:
    ...
    for g in data.get("games", []):
        ...
        game = g.get("game", "")
        sel = g.get("selected") or {}
        ad_key = sel.get("ad_key", "")[:12]
        files = find_downloaded_files(company, game, ad_key)
        video_url = None
        for r in sel.get("resource_urls", []):
            if r.get("video_url"):
                video_url = r["video_url"]
                break
        assets_map[f"{company}_{game}"] = {
            "files": files,
            "video_url": video_url,
            "ad_key": ad_key,
        }
...
def main():
    ...
    prompt = f"""请根据以下竞品 UA 素材数据，生成一份**UA 素材日报**（日期：{date_str}）。
...
- **在介绍每个游戏时，请使用 Markdown 图片语法引用已下载的封面图**（格式：![游戏名封面](../ua_downloads/公司_游戏_adkey_cover.jpg)）
...
请直接输出日报正文，不要输出其他说明。"""
    ...
    header = f"""# UA 素材日报
**日期**: {date_str}  
**素材来源**: 广大大（{len(data.get('games', []))} 款竞品游戏）
