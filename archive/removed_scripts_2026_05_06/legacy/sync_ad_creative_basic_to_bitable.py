"""
将 SQLite 中的 ad_creative_analysis 表同步到飞书多维表格（只用中文标题/正文，并上传封面图）。
...
用法（项目根目录）：
  python scripts/sync_ad_creative_basic_to_bitable.py
"""
...
# 仅用于 basic 表同步：强行写死到基础创意多维表（不影响其它脚本）
...
def fetch_ad_creatives(conn: sqlite3.Connection, batch_size: int):
    """
    以批次方式读取 ad_creative_analysis 中的所有记录。
    这里不做日期筛选，由多维表格端自行筛选/视图控制。
    """
...
def _create_field(
    ...
    if resp.status_code != 200 or data.get("code") != 0:
        print(
            f"[sync_basic] 创建字段失败（{field_name}），"
            f"status={resp.status_code}, resp={data}"
        )
    else:
        print(f"[sync_basic] 已创建字段：{field_name}")
...
def batch_create_basic_records(
    ...
    data = resp.json()
    if data.get("code") != 0:
        raise RuntimeError(f"batch_create (basic) failed: {data}")


def main() -> None:
    token = get_tenant_access_token()
    # 先确保 basic 表中有我们需要的字段
    ensure_basic_fields(token)

    conn = sqlite3.connect(DB_PATH)
    total_sent = 0
    try:
        for batch_idx, rows in enumerate(fetch_ad_creatives(conn, BATCH_SIZE), start=1):
            records: List[Dict[str, Any]] = []
            for row in rows:
            ...
            print(
                f"[sync_ad_creative_basic] 已同步批次 {batch_idx}，"
    ...
    print("[sync_ad_creative_basic] 同步完成，总计写入记录数：", total_sent)
