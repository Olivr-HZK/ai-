...
#   ./scripts/daily_arrow2_workflow.sh crawl-only         # 仅爬取+落盘：默认 latest_yesterday（7天最新创意+仅昨日 first_seen，与 config 中「最新」那组一致）
#   ./scripts/daily_arrow2_workflow.sh crawl-only exposure_top10   # 仅爬取+落盘，改用展示估值那一组
#   仅爬 + 有头浏览器 + 每产品打印素材、按 Enter 继续（**默认 latest_yesterday**；脚本已带 --debug）：
...
#
# 地区补全：run_arrow2_batch 内未设置时默认 开启 + DOM 点卡（见 run_search_workflow run_arrow2_batch）。
# 显式关闭：ARROW2_ENRICH_DETAIL_COUNTRY=0 或 ARROW2_ENRICH_DETAIL_COUNTRY_DOM=0
#
# 可选环境变量：
#   DEBUG=1
...
case "$MODE" in
  ...
  crawl-only)
    ...
    exec "$PY" scripts/test_arrow2_competitors.py \
  ...
  crawl-pause|crawl-pause-per-product|crawl-interactive)
    ...
    # 与 test_arrow2_competitors 一致：--debug-step-products = 有头 + 每搜索词后打印素材摘要并 Enter 下一词
    export DEBUG=1
    exec "$PY" scripts/test_arrow2_competitors.py \
      --output-prefix "${PREFIX}_${PULL_SUB}" \
      --pull-only "$PULL_SUB" \
    ...
    ;;
  *)
    echo "用法: $0 [all | latest_yesterday | exposure_top10 | crawl-only [exposure_top10|latest_yesterday] [test_arrow2 参数] | crawl-pause [exposure_top10] …]（默认 all=展示估值；crawl-only/crawl-pause 无参时=latest_yesterday）" >&2
    exit 1
    ;;
esac
