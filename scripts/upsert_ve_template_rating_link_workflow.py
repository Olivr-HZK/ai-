#!/usr/bin/env python3
"""Create or update the Feishu Base rating-trigger workflow for VE template links."""
...
def _write_payload_file(payload: dict[str, Any]) -> str:
    ...
    path = target_dir / "ve_template_rating_link_workflow_payload.tmp.json"
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return "data/ve_template_rating_link_workflow_payload.tmp.json"


def _normalize_ensure_link_url(value: str) -> str:
    ...
    if url.endswith("/trigger"):
        return f"{url[: -len('/trigger')]}/ensure-link"
    return f"{url}/ensure-link"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="创建或更新 VE 评分自动写触发链接 Workflow")
    parser.add_argument("--ensure-link-url", default="", help="公网可访问的 https://.../ensure-link")
    parser.add_argument("--public-url", default="", help="公网根 URL 或 /trigger URL，会自动转换为 /ensure-link")
    ...
    parser.add_argument("--title", default="", help="默认 VE 模板复刻评分链接写入（评分字段名）")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ensure_link_url = _normalize_ensure_link_url(
        args.ensure_link_url
        or args.public_url
        or os.getenv("VE_TEMPLATE_PUBLIC_ENSURE_LINK_URL", "")
        or os.getenv("VE_TEMPLATE_PUBLIC_TRIGGER_URL", "")
        or os.getenv("VE_TEMPLATE_TRIGGER_URL", "")
    )
    if not ensure_link_url:
        raise SystemExit("请传入 --ensure-link-url 或 --public-url")
    base_token = parse_bitable_ref(bitable_url).app_token
    title = args.title or f"VE 模板复刻评分链接写入（{args.rating_field_name}）"
    payload = build_rating_link_workflow_payload(
        ensure_link_url=ensure_link_url,
