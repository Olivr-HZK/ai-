...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Video Enhancer 爬取冒烟测试：只跑一个产品，只落 raw JSON，不入库/不同步。",
        epilog=(
            "示例:\n"
...
def main() -> None:
    ...
    print(f"[smoke] Video Enhancer crawl-only product={product!r} date={date} output_prefix={output_prefix}")
