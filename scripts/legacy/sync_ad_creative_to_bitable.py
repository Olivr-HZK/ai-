...
BITABLE_FIELD_NAMES = {
    ...
    "封面图链接",
    "封面图",  # 附件字段，用于实际显示图片
    "AI分析结果",  # 保留原有素材级 LLM 分析
    "UA灵感借鉴",  # 新增：按产品的 UA 建议
    "抓取日期",
    "创建时间",
    "更新时间",
    "视频时长",
...
def row_to_fields(
    ...
    ua_suggestion_override: str | None = None,
) -> Dict[str, Any]:
    """
    ...
    # 若调用方未显式传入我方产品 / 建议且行中自带列，则从行中取
    row_keys = set(row.keys())
    if our_product is None and "our_product" in row_keys:
        val = row["our_product"]
        our_product = str(val) if val is not None else None
    if ua_suggestion_override is None and "ua_suggestion" in row_keys:
        val = row["ua_suggestion"]
        ua_suggestion_override = str(val) if val is not None else None
    ...
    # 我方产品：一条素材一条产品
    if our_product and "我方产品" in BITABLE_FIELD_NAMES:
        fields["我方产品"] = our_product

    # UA 灵感借鉴：按产品级 UA 建议，单独放在新字段里，保留原有 AI分析结果 不变
    if ua_suggestion_override and "UA灵感借鉴" in BITABLE_FIELD_NAMES:
        fields["UA灵感借鉴"] = ua_suggestion_override
...
def fetch_rows_in_batches(conn: sqlite3.Connection, batch_size: int):
    """
    以批次方式从 ad_creative_analysis + ad_creative_product_suggestion 联合读取数据。
    每条记录 = 一条素材 × 一条我方产品 × 一条 UA 建议。
    """
