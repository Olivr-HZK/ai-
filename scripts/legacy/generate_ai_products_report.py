...
from path_util import CONFIG_DIR, DATA_DIR, REPORTS_DIR, DOWNLOADS_DIR

INPUT_FILE = DATA_DIR / "ai_products_ua_results.json"
OUTPUT_FILE = REPORTS_DIR / "ai_products_report_daily.md"

# 产品手册 CSV（用于基于产品的精准推荐）
PRODUCT_MANUAL_CSV = CONFIG_DIR / "产品手册_AI工具类_表格 2.csv"


def load_product_manual_summary() -> str:
    """
    读取产品手册 CSV，生成供 LLM 参考的「我方产品」摘要。
    列：名称, 图标, 分类, 内部名称, 编号, 产品描述, 竞品 等。
    """
    if not PRODUCT_MANUAL_CSV.exists():
        return ""
    lines = []
    try:
        with open(PRODUCT_MANUAL_CSV, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if not name and not internal:
                    continue
                lines.append(
                    f"- **{name or internal}**（内部名称: {internal or name}）| 分类: {category} | 产品描述: {desc} | 竞品: {competitors}"
                )
...
def main():
    ...
    product_manual = load_product_manual_summary()

    prompt_parts = [
        f"请根据以下竞品 AI 产品 UA 素材数据，生成一份**AI 产品 UA 素材日报**（日期：{date_str}）。",
        "",
        "## 素材数据",
        summary,
    ]
    if product_manual:
        prompt_parts.extend(["", "## 我方产品手册（供精准推荐参考）", product_manual])
    prompt_parts.extend([
        "",
        "## 日报要求",
        ...
        "",
        "### 二、基于素材：适合的我方产品",
        "以「素材数据」中的**每条竞品素材**为主线，列出该素材适合「我方产品手册」中的哪几款产品使用，并简要说明理由（一句话即可）。",
        "格式示例：对每个竞品素材写「**竞品名 / 分类** → 适合我方产品：产品A（理由）；产品B（理由）。」",
        "这样读者可以先从素材出发，快速看到我们有哪些产品可以复用该素材或创意。",
        "",
        "### 三、基于各产品的 UA 建议",
        "针对「我方产品手册」中的**每一款我方产品**，按下列 5 点给出建议（保持与当前格式一致）：",
        "1. **与该产品最相关的竞品素材**（从素材数据中指名具体竞品/广告主，说明为何相关）",
        "2. **可借鉴的创意方向**（结合该产品定位与竞品表现，至少 2 条）",
        "3. **可复用的文案/卖点句式**",
        "4. **投放策略建议**（平台、时长、节奏）",
        "5. **避坑提示**（需注意的风险或失效套路）",
        "",
