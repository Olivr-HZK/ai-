...
from sync_raw_analysis_to_bitable_and_push_card import _render_card_markdown
...
WECOM_MAX_TEXT_BYTES = 3800  # 参考企业微信单条消息长度上限 4096；预留前缀等开销
...
def _split_wecom_blocks_by_direction(text: str) -> List[str]:
    ...
    for line in lines:
        ...
        if not in_common and direction_re.match(line):
            flush()
            cur.append(line)
            continue
...
def _transform_wecom_text(text: str) -> str:
    # 隐藏“参考链接”标签，只保留可点击的视频链接
    text = text.replace("🔗 参考链接：", "")

    # 将 markdown 链接 [视频1](http...) 改成 视频1：http...
    # 兼容多个分隔符：视频1：url；视频2：url
    text = re.sub(r"\[视频(\d+)\]\((https?://[^)]+)\)", r"视频\1：\2", text)
    # 其他 markdown 链接统一改为“文案：url”，保证企业微信 text 下可点击
...
def main() -> None:
    ...
    # 统一两边文案：使用飞书同款渲染函数（只是不解析 markdown）
    ...
    if not args.sheet_only:
        try:
            ...
            print(f"[wecom] markdown 推送失败，回退 text：{e}")
            push_wecom_text(wecom_webhook, summary_text)
