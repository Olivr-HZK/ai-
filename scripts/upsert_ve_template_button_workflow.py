#!/usr/bin/env python3
"""Create or update the Feishu Base button workflow for VE template triggering."""
...
DEFAULT_WORKFLOW_TITLE = "VE 模板复刻按钮触发"
...
def _write_payload_file(payload: dict[str, Any]) -> str:
    ...
    path = target_dir / "ve_template_button_workflow_payload.tmp.json"
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return "data/ve_template_button_workflow_payload.tmp.json"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="创建或更新 VE 模板复刻按钮 Workflow")
    parser.add_argument("--trigger-url", required=True, help="公网可访问的 https://.../trigger")
    parser.add_argument("--bitable-url", default="", help="默认 VIDEO_ENHANCER_BITABLE_URL")
    parser.add_argument("--table-name", default=DEFAULT_TABLE_NAME)
    parser.add_argument("--title", default=DEFAULT_WORKFLOW_TITLE)
...
def main(argv: list[str] | None = None) -> int:
    ...
    trigger_url = args.trigger_url.rstrip("/")
    if not trigger_url.endswith("/trigger"):
        trigger_url = f"{trigger_url}/trigger"
    ...
    workflow_id = _find_workflow(base_token, args.title, args.identity)
    if args.dry_run:
        print(
            json.dumps(
