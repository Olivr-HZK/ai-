"""
根据 operation.json 中的元素执行搜索工作流（skills 自包含版）。

流程：登录 → 搜索 → 时间框选7天 → 广告素材框选素材 → 筛选框选展示估值
结果：从素材内容中选 天数最新 且 展示估值最高 的素材

登录后跳转的页面即有搜索框，用 operation.json 中的 HTML 匹配元素（若存在）。
"""
import argparse
import asyncio
import json
...
from guangdada_login import login  # noqa: E402
from path_util import CONFIG_DIR, DATA_DIR  # noqa: E402
...
def _load_selectors():
    """从 operation.json 加载，用 HTML 解析选择器；不存在则用默认选择器"""
    sel_map = {}
    if OP_FILE.exists():
        try:
